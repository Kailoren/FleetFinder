...
- **Find Carriers**, a picker of every tradeable Odyssey component, showing how many you currently hold. Tick the ones you're short on and hit search. Results are grouped one row per carrier, showing what they sell, the price, the distance from your current system, and how recently the listing was updated. Click a system name to copy it to the clipboard for the in-game galaxy map.
...
- **Modifications**, every suit and weapon modification and suit upgrade with the components it needs. Tick the ones you want (with a quantity stepper per mod) and **Apply selected**, this sets a target on the Find Carriers tab for everything those mods require and auto-selects whatever you're still short on, so you can go straight to searching.
...
Your inventory (the "Have" column) is read straight from the game and refreshes automatically while you play, so it stays accurate without you doing anything.
...
- The component catalogue is curated for suit and weapon engineering, not every Odyssey material that exists. If an EDOMH import shows an item it doesn't recognize, that's flagged in the Import tab so you can see exactly what wasn't matched. If this happens, please let us know so it can be added.
